...
async function call(method: string, query: BehanceQuery = {}): Promise<any> {
  query.api_key = TOKEN;
  try {
    const result = await axios.get(`${BASE_URL}${method}?`, { timeout: 10000, params: query });
...
export async function projects(time = 'all', sort = ''): Promise<{ projects: BehanceProject[] } | null> {
