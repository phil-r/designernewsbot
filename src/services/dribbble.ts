...
async function addDribbbleShot(shot: DribbbleShot) {
  ...
  const exisitngShot = await prisma.dribbbleShot.findUnique({ where: { id: shot.id } });
  if (exisitngShot) {
    cache.set(cacheKey, shot.url);
    return;
  }
  const shortUrl = `${development ? 'http://localhost:8080' : 'https://dsgnr.news'}/d/${shortId}`;
  ...
  const result = shot.video ?
    await sendVideo(TELEGRAM_CHANNEL, shot.video, message, { 'inline_keyboard': [buttons] }) :
    await sendPhoto(TELEGRAM_CHANNEL, shot.img, message, { 'inline_keyboard': [buttons] });
...
export async function dribbbleCron() {
  ...
  console.log('checking shots:', shotsList.map(shot => shot.id));
